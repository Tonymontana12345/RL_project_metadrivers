...
def evaluate_model(model_path, test_seeds, n_episodes=20, render=False, verbose=True):
    ...
    for seed in test_seeds:
        ...
        for episode in iterator:
            ...
            total_reward = 0.0
            steps = 0
            max_steps = 2000  # 최대 스텝 제한 (horizon과 동일)
            last_info = info
            
            # 첫 번째 에피소드에서만 info 키 확인 (디버깅)
            if episode == 0 and verbose:
                print(f"\n  🔍 디버깅: info 키 = {list(info.keys())}")
            ...
            while not done and steps < max_steps:
                ...
                steps += 1
                last_info = info  # 마지막 step info 저장
                
                # 첫 번째 에피소드의 처음 몇 스텝만 상세 로깅
                if episode == 0 and steps <= 5 and verbose:
                    print(f"    스텝 {steps}: 보상={float(reward):.4f}, 누적={total_reward:.4f}, done={done}")
            
            # 최대 스텝 도달 시 경고
            if steps >= max_steps and verbose and episode == 0:
                print(f"  ⚠️  경고: 최대 스텝({max_steps}) 도달")
            
            # 결과 기록
            episode_rewards.append(total_reward)
            episode_lengths.append(steps)
            
            # ✅ 에피소드 단위로 성공/충돌/이탈 여부 판단 (마지막 info 기준)
            info_flag = last_info if last_info is not None else {}
            
            success = bool(info_flag.get("arrive_dest", False) or
                           info_flag.get("arrive_destination", False))
            crash = bool(info_flag.get("crash", False) or
                         info_flag.get("is_crashed", False) or
                         info_flag.get("crashed", False))
            out_of_road = bool(info_flag.get("out_of_road", False))
            ...
            # 첫 번째 에피소드 상세 정보 출력 (디버깅)
            if episode == 0 and verbose:
                print(f"  📊 첫 에피소드: 보상={total_reward:.2f}, 스텝={steps}, done={done}")
                print(f"  📋 마지막 info 키: {list(info_flag.keys()) if info_flag else 'empty'}")
                print(f"  ✅ 에피소드 종료 플래그: success={success}, crash={crash}, out_of_road={out_of_road}")
    ...
    if verbose:
        ...
        if results['summary']['overall_success_rate'] == 0:
            ...
            print("   3. 에이전트가 항상 도로를 벗어나거나, 도착 조건을 만족하지 못함")
            print("\n💡 해결 방법:")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default=f"models/td3_fixed_seed_{FIXED_SEED}.zip",
        help="평가할 모델 경로"
    )
    parser.add_argument(
        "--seeds",
